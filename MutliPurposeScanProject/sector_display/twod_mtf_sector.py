...
def regenerate_folder_section(frame, database_folder):
    ...
    database_loc = database_folder.replace("\\", "/")+"/database_MTF.csv"
    df_x = pd.read_csv(database_loc)
    df_empty = df_x.iloc[0:0]
    df_empty.to_csv(database_loc, index=False)


    activate_required_mtf(database_folder, database_loc)
    regenerated_respose = mtf_section_builder(frame, regenerate_old=True)


def mtf_canvas_builder(frame):
...
def mtf_section_builder(frame, regenerate_old=False, *args):
    global list_of_section
    ...
    for folder in os.listdir(path):
        if "Mean" != folder:
            ...
            df = pd.read_csv(os.path.join(folder_location, f"database_{kind}.csv"))
            ...
            if kind == "MTF":
                ...
                for index, row in df.iterrows():
                    ...
                    avg_mtf = ast.literal_eval(row["avg_mtf"])
                    spatial_frequency = ast.literal_eval(row["spacial_frequency"])
                    
                    fig = Figure(figsize=(6, 4), dpi=100)  # Adjust figsize for better aspect ratio
                    plot = fig.add_subplot(111)
                    plot.plot(spatial_frequency[:len(spatial_frequency)//2], avg_mtf[:len(avg_mtf)//2], label="MTF Curve")
                    
                    plot.set_title("Mean MTF Curve - " + row["circle_name"])
